...
// Extracted from falltax-us/index.html · 138669 bytes of source logic
...
// Sovereign return management + UK law engine for SRA-regulated firms
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Tax opinion signed and retained per SRA 13.5',{id:a.id,returnId:a.returnId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Falltax</strong> is a tool for SRA-regulated UK solicitors. It assists with return management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or tax opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign return management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>paratax</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue tax opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
async function askT3(){
 ...
 const sys='You are a UK tax research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated tax advice — research only. Always say "verify against primary sources".';
